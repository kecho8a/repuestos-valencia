    ...
    <>
      ...
      <nav id="bottom-bar" className="fixed bottom-0 left-0 right-0 z-50 h-16 bg-white/75 backdrop-blur-md border-t border-zinc-200 px-4 flex items-center justify-between md:max-w-md md:mx-auto md:rounded-t-2xl md:border-x border-zinc-200 shadow-2xl lg:hidden">
        ...
        <button
          ...
          className="flex flex-col items-center justify-center flex-1 py-1 text-zinc-500 hover:text-[#3b82f6] transition-colors relative"
        >
          <Menu size={20} />
        ...
        <button
          type="button"
          onClick={() => setTab('home')}
          className={`flex flex-col items-center justify-center flex-1 py-1 transition-colors relative ${currentTab === 'home' ? 'text-[#3b82f6] font-semibold' : 'text-zinc-500'}`}
        >
          {currentTab === 'home' && (
            <span className="absolute -top-1 w-5 h-[2px] bg-[#3b82f6] rounded-full" />
          )}
          <Home size={20} />
        ...
        {isAdminAuthenticated && (
          <button
            ...
            className={`flex flex-col items-center justify-center flex-1 py-1 transition-colors relative ${currentTab === 'admin' ? 'text-[#3b82f6] font-semibold' : 'text-amber-600'}`}
          >
            <ShieldAlert size={20} />
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center flex-1 py-1 transition-colors relative ${currentTab === 'catalog' ? 'text-[#3b82f6] font-semibold' : 'text-zinc-500'}`}
        >
          {currentTab === 'catalog' && (
            <span className="absolute -top-1 w-5 h-[2px] bg-[#3b82f6] rounded-full" />
          )}
          <Grid size={20} />
          <span className="text-[10px] mt-0.5 font-display font-medium">Catálogo</span>
        </button>
        ...
        <button
          type="button"
          onClick={() => setTab('cart')}
          className={`flex flex-col items-center justify-center flex-1 py-1 transition-colors relative ${currentTab === 'cart' ? 'text-[#3b82f6] font-semibold' : 'text-zinc-500'}`}
        >
          {currentTab === 'cart' && (
            <span className="absolute -top-1 w-5 h-[2px] bg-[#3b82f6] rounded-full" />
          )}
          <div className="relative">
            ...
            {cartCount > 0 && (
              <span className="absolute -top-1.5 -right-2 bg-gradient-to-r from-red-550 to-indigo-600 text-white font-mono font-bold text-[9px] w-4 h-4 rounded-full flex items-center justify-center animate-bounce">
                {cartCount}
              </span>
            )}
          </div>
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center flex-1 py-1 transition-colors relative ${currentTab === 'profile' ? 'text-[#3b82f6] font-semibold' : 'text-zinc-500'}`}
        >
          {currentTab === 'profile' && (
            <span className="absolute -top-1 w-5 h-[2px] bg-[#3b82f6] rounded-full" />
          )}
      ...
      >
        ...
        {/* Sliding Drawer Container (Skins: Left slide iOS side drawer, 280px wide) */}
        <aside 
        ...
        >
          ...
          <div className="overflow-y-auto max-h-[85vh] no-scrollbar text-zinc-900">
            ...
            <div className="p-4 bg-gradient-to-r from-zinc-50 to-zinc-100/40 border-b border-zinc-200 flex justify-between items-center">
              <div className="flex flex-col">
                <span className="font-display font-bold text-lg text-zinc-900 tracking-tight flex items-center gap-1.5">
                  <span className="text-[#3b82f6]">⚙️</span> {config.site_nombre}
                </span>
                <span className="text-[9px] text-[#3b82f6] font-mono tracking-wider">Valencia - Carabobo</span>
              </div>
              <button
                type="button"
                onClick={() => setDrawerOpen(false)}
            ...
            <div className="p-3 flex flex-col gap-1">
              ...
              <button
                ...
                className={`flex items-center gap-3 w-full px-3 py-2 text-xs rounded-lg font-semibold transition-all cursor-pointer ${currentTab === 'home' ? 'bg-blue-50 text-[#3b82f6]' : 'text-zinc-700 hover:bg-zinc-100'}`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-3 w-full px-3 py-2 text-xs rounded-lg font-semibold transition-all cursor-pointer ${currentTab === 'catalog' ? 'bg-blue-50 text-[#3b82f6]' : 'text-zinc-700 hover:bg-zinc-100'}`}
              >
                <Grid size={15} /> Catálogo e indexaciones
              </button>

              <button
                type="button"
                onClick={() => { setTab('cart'); setDrawerOpen(false); }}
                className={`flex items-center gap-3 w-full px-3 py-2 text-xs rounded-lg font-semibold transition-all cursor-pointer ${currentTab === 'cart' ? 'bg-blue-50 text-[#3b82f6]' : 'text-zinc-700 hover:bg-zinc-100'}`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-3 w-full px-3 py-2 text-xs rounded-lg font-semibold transition-all cursor-pointer ${currentTab === 'profile' ? 'bg-blue-50 text-[#3b82f6]' : 'text-zinc-700 hover:bg-zinc-100'}`}
              >
            ...
            <div className="p-3 border-t border-zinc-200 flex flex-col gap-1">
              ...
              <button
                ...
                className="flex items-center gap-3 w-full px-3 py-2 text-xs text-zinc-700 hover:bg-zinc-100 rounded-lg font-semibold transition-all cursor-pointer"
              >
                <span className="text-base text-blue-500">📷</span> Escáner Barcode / OEM
              </button>
              ...
              <a
                ...
                className="flex items-center gap-3 w-full px-3 py-2 text-xs text-zinc-800 hover:bg-emerald-50 hover:text-emerald-600 rounded-lg font-semibold transition-all"
              >
                <PhoneCall size={15} className="text-emerald-600" /> Contactar WhatsApp Directo
              </a>
            </div>
          ...
          <div className="p-4 border-t border-zinc-200 bg-zinc-50">
            ...
            <div className="text-[9px] text-zinc-400 font-mono text-center mt-2">
              TuRepuestoValencia v1.0.1 PRPROD
            </div>
          </div>
        </aside>
      </div>
