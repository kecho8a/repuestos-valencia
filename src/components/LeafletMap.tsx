...
export const LeafletMap: React.FC<LeafletMapProps> = ({ onLocationSelected, shopCoords, config }) => {
  ...
  useEffect(() => {
    ...
    // Create customized Shop Pin Icon (Sleek High Contrast Blue/Charcoal)
    const shopIcon = L.divIcon({
      html: `
        <div class="relative flex items-center justify-center">
          <span class="absolute inline-flex h-8 w-8 rounded-full bg-blue-600 opacity-25 animate-ping"></span>
          <div class="relative bg-zinc-950 border border-[#0060df] text-[#0060df] p-2 rounded-full flex items-center justify-center shadow-md">
    ...
    // Create customized User Pin Icon (Samsung Blue)
    const userIcon = L.divIcon({
      html: `
        <div class="relative flex items-center justify-center">
          <span class="absolute inline-flex h-8 w-8 rounded-full bg-blue-600 opacity-20 animate-pulse"></span>
          <div class="relative bg-[#0060df] border-2 border-white text-white p-2 rounded-full flex items-center justify-center shadow-lg">
    ...
    L.marker([shopCoords.lat, shopCoords.lng], { icon: shopIcon })
      ...
      .bindPopup(`
        <div class="text-xs p-1 font-sans text-zinc-900">
          <h4 class="font-bold text-[#0060df] mb-0.5">Sede TuRepuestoValencia</h4>
  ...
  return (
    <div className="flex flex-col gap-4 font-sans text-zinc-900">
      ...
      <div className="flex items-center justify-between py-2.5 px-3.5 border border-zinc-200 rounded-lg bg-zinc-50 text-xs text-zinc-850 font-mono shadow-sm">
        ...
        <span className="text-blue-700 font-extrabold uppercase tracking-wide">{zone || "Estableciendo dirección..."}</span>
      </div>
    </div>
  );
};
